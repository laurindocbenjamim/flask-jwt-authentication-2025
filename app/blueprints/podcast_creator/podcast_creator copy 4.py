...
font_file_path = os.path.join(os.path.dirname(__file__), 'fonts/Inter', 'Inter-VariableFont_opsz,wght.ttf')
...
class PodcastGenerate(Resource):
    ...
    def post(self):
        ...
        temp_frames_dir = os.path.join(app.root_path, 'static', app.config['TEMP_FRAMES_FOLDER'], str(uuid.uuid4()))
        ...
        try:
            ...
            # Initialize final_video_clip with background and waveform, so it's always defined
                ...
                try:
                    if not os.path.exists(font_file_path):
                        ...
                        try:
                            ...
                            logging.error(f"Pillow failed to load font {font_file_path}: {font_error}")
                            logging.warning("Text overlay will be skipped due to font error.")
                except Exception as e:
                        logging.error(f"Failed to create TextClip : {e}", exc_info=True)
